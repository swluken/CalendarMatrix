 ...
 */
 Ext.define('Ext.calendar.CalendarPanel', {
    ...
    showMonthView: true,
    /**
     * @cfg {Boolean} showYearView
     * True to include the year view (and toolbar button), false to hide them (defaults to true).
     */
    showYearView: true,	
    /**
    ...
    /**
     * @cfg {String} yearText
     * Alternate text to use for the 'Year' nav bar button.
     */
    yearText: 'Year',
    ...
    initComponent: function() {
        ...
        if (this.showDayView) {
            ...
            this.add(day);
			this.day = this.layout.getLayoutItems().slice(-1).pop();
        }
        if (this.showWeekView) {
            var wk = Ext.applyIf({
                xtype: 'weekview',
            ...
            this.add(wk);
			this.wk = this.layout.getLayoutItems().slice(-1).pop();
        }
        if (this.showMonthView) {
            ...
            this.add(month);
			this.month = this.layout.getLayoutItems().slice(-1).pop();
        }
        ...
        if (this.showYearView) {
          ...
          //  this.initEventRelay(year);
            this.add(year);
			this.year = this.layout.getLayoutItems().slice(-1).pop();
        }		
    ...
    initEventRelay: function(cfg) {
        ...
        cfg.listeners.afterrender = {
            fn: function(c) {
                ...
                'eventmove', 'datechange', 'rangeselect', 'eventdelete', 'eventresize', 'initdrag', 'calendarselect', 'monthselect']);
            },
            scope: this,
            single: true
        };
    ...
    onPrevClick: function() {
        this.startDate = this.layout.activeItem.movePrev(this);
        this.updateNavState();
        this.fireViewChange();
    },
    ...
    onNextClick: function() {
        this.startDate = this.layout.activeItem.moveNext(this);
        this.updateNavState();
        this.fireViewChange();
    },
