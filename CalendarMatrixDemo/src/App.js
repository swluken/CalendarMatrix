                ...
                items: [{
                    ...
                    items: [{
                        ...
                        listeners: {
                            'select': {
                                fn: function(dp, dt){
									// Modified logic to update all tabs when date is selected
									var cp = Ext.getCmp('app-calendar');
									cp.startDate =  dt;
									if (cp.day){cp.day.setStartDate(dt, true);}
									if (cp.wk){cp.wk.setStartDate(dt, true);}
									if (cp.month){cp.month.setStartDate(dt, true);}
									if (cp.year){cp.year.setStartDate(dt, true);}
                                    cp.setStartDate(dt);
                                },
                                scope: this
                            }
